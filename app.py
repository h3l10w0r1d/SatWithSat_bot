...
from stats import (
    ...
    maybe_award_streak_saver,
    streak_days_with_saver,
)
from admin import (
    admin_help,
    list_pending_users,
...
# debug capture
_LAST_ERROR = {"req_id": None, "error": None}
...
def set_last_error(req_id: str, err: str):
    _LAST_ERROR["req_id"] = req_id
    _LAST_ERROR["error"] = err
...
def webhook():
    ...
    try:
        ...
        user = get_user_by_tg(telegram_id) or user  # refresh
        ...
        if incoming.lower().startswith("/start"):
            # IMPORTANT FIX: only reg_step decides registration, not registered_at
            if user.reg_step != 0:
