...
def top_improvers(limit: int = 10) -> str:
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id, first_name, surname, nickname FROM users WHERE approved=TRUE AND banned=FALSE")
            users = cur.fetchall()
...
def export_csv(chat_id: int) -> None:
    ...
    w = csv.writer(buf)
    w.writerow([
        "telegram_id","first_name","surname","nickname","email","approved","banned",
        "tests_count","total_points","goal_math","streak_savers","last_test_at"
    ])
    for r in rows:
        w.writerow([
